...
export default function CreateBotClient() {
  ...
  const handleNext = async () => {
    if (currentStep === 0) {
      ...
      let success = false;
      try {
        let body: any = { role: selectedRole, botName: botName || "AI Assistant" };
        ...
        if (sourceType === "url") {
            ...
            const base64Promise = new Promise<string>((resolve, reject) => {
                readerBase64.onload = () => resolve((readerBase64.result as string).split(',')[1]);
                readerBase64.onerror = reject;
            });
            readerBase64.readAsDataURL(pdfFile);
            
            body.pdfBase64 = await base64Promise;
      ...
      try {
        const result = await createBot({
            ...
            role: selectedRole,
            url: sourceType === "pdf" ? (pdfFile?.name || "PDF Document") : url,
        ...
        <div className="bg-white shadow-sm border border-slate-200 rounded-xl overflow-hidden flex flex-col min-h-[500px]">
            <div className="flex-1 p-8 md:p-12">
                ...
                {currentStep === 0 && (
                    <div className="animate-in fade-in slide-in-from-right-4 duration-500 h-full flex flex-col justify-center">
                        ...
                        <p className="text-slate-500 mb-8 text-center">Provide a website URL or PDF file for the AI to learn your business.</p>
                        ...
                        <div className="max-w-md mx-auto w-full space-y-6">
                            ...
                            <div>
                                <label className="text-sm font-semibold text-slate-700 mb-4 block text-center">Select Knowledge Source</label>
                                <div className="flex p-1 bg-slate-100 rounded-xl mb-8 max-w-[280px] mx-auto border border-slate-200">
                                    <button 
                                        onClick={() => setSourceType("url")}
                                        className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-bold transition-all ${sourceType === "url" ? "bg-white text-indigo-600 shadow-sm border border-slate-200" : "text-slate-500 hover:text-slate-700"}`}
                                    >
                                        <Globe size={16} /> Website
                                    </button>
                                    <button 
                                        onClick={() => setSourceType("pdf")}
                                        className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-bold transition-all ${sourceType === "pdf" ? "bg-white text-indigo-600 shadow-sm border border-slate-200" : "text-slate-500 hover:text-slate-700"}`}
                                    >
                                        <Bot size={16} /> PDF File
                                    </button>
                                </div>
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <div className="relative group">
                                            <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                                <Globe className="text-slate-400 group-focus-within:text-indigo-500 transition-colors" size={18} />
                                            </div>
                                            <input 
                                                type="text" 
                                                value={url}
                                                ...
                                                className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-12 pr-4 py-4 focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all text-slate-700 font-medium"
                                            />
                                        </div>
                                    </div>
                                ) : (
                                    <div className="space-y-2">
                                        <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Upload PDF</label>
                                        <div className="relative group">
                                            <input 
                                                type="file" 
                                                accept=".pdf"
                                            ...
                                            <label 
                                                htmlFor="pdf-upload"
                                                className="w-full bg-slate-50 border-2 border-dashed border-slate-200 rounded-xl p-8 flex flex-col items-center justify-center cursor-pointer hover:border-indigo-400 hover:bg-indigo-50/30 transition-all group-focus-within:border-indigo-500"
                                            >
                                                ...
                                                <span className="text-slate-400 text-xs font-medium">
                                                    {pdfFile ? `${(pdfFile.size / 1024 / 1024).toFixed(2)} MB` : "Max size 10MB"}
                                                </span>
                                            </label>
                                        </div>
                                    </div>
            ...
            <div className="bg-white p-6 border-t border-slate-100 flex justify-between items-center px-8 md:px-12">
                ...
                <div className="flex items-center gap-6">
                    ...
                    <Button 
                        onClick={handleNext}
                        disabled={(currentStep === 0 && (sourceType === "url" ? !url : !pdfFile)) || isAnalyzing}
